...
## 2026-08-27 — Collective confirm card handles rooms already in your pipeline
Nick: the "Already in the collective" confirm card offered "Looks right — start my deal (+1)" even when you ALREADY have a deal on that room — which duplicates, or errors on unique(artist_id, venue_id). Fix: new myDealForVenue(venueId) helper (checks primary venueId AND dealVenueMap secondary-in-play, returns {idx, secondary} or null). When you already have the deal: the confirm card shows a blue "You're already working this room" banner, the primary button becomes "Open my deal (+1)" (routes to the existing deal instead of addVenueToPipeline), and the collective search results badge the row "· in your pipeline" so it's visible before tapping. The +1 freshness verify still fires either way — confirming the info is right is valuable whether or not it's a new deal. Backstop: addVenueToPipeline's insert error still toasts if a stale pipeline array slips through.
## 2026-08-27 — Phantom follow-up flash: the REAL root cause (updateHomeStats render before gate reset)
Third and actual fix for the phantom-nudge flash (the render gate + loadSeq were necessary but insufficient). Mechanism: loadPipeline's .then rebuilt `pipeline` with activity:[] (empty), then called updateHomeStats() — which ENDS in renderHome() — and ONLY AFTER that set actsReady=false/schedReady=false. So at the updateHomeStats→renderHome instant, the flags still read "ready" from the PRIOR load, the gate passed, and it painted every pitched deal ≥7d quiet as follow-up-due (snooze/dismiss markers not loaded yet → followupDue true for all), then collapsed to the true 1 when loadActivities landed. loadSeq didn't catch it because it's synchronous within the same seq. Fix: move `actsReady=false; schedReady=false;` to BEFORE the pipeline rebuild, so updateHomeStats→renderHome hits the gate and holds the previous TRUE frame (el.innerHTML non-empty on reload → early return, no skeleton flash) until both loaders complete. Lesson: any renderHome() reachable synchronously between pipeline-reassignment and loader-dispatch must be behind the gate — set gate flags at the very top of a reload, never mid-way.
## 2026-08-27 — Split "Winery / Brewery" into "Vineyard / Winery" + "Distillery / Brewery"
Nick: a vineyard and a distillery are different rooms. softTypes now lists both separately. venueTypeFromGoogle maps winery/vineyard → "Vineyard / Winery", brewery/distillery/brewpub/beer_hall → "Distillery / Brewery". venueIcon differentiates: vineyard/winery → ti-bottle, brewery/distillery → ti-barrel. roomClass's /winery|vineyard|brewery|distillery/ already catches both new labels → same "winery" pitch playbook (patio/daytime/drinks crowd fits both). Existing rows stored as old "Winery / Brewery" still render (brewery substring → barrel) and pitch fine; not auto-split since the old label is ambiguous.
## 2026-08-27 — Conversation bubbles render markdown links (were showing raw [text](url))
Nick screenshotted a sent bubble showing literal "[epk.nickrainsmusic.com](https://epk.nickrainsmusic.com)". The bubble rendered content with esc(text) — escapes but doesn't convert the [text](url) syntax the AI writes for the listen/EPK link (the worker's mdToHtml converts it for the actual EMAIL, but the in-app view didn't match). Fix: bubble content now uses mdRich(text) (existing helper: esc + MD_LINK → <a>), so the link renders clickable in-app exactly as it goes out. Summary stays esc (plain AI one-liner). MD_LINK requires https:// which the AI's links carry.
