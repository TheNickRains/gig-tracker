...
**Locked scheduled draft** — once a send is scheduled the editor is replaced by a read-only "Scheduled draft" card: lock header ("Locked — sends exactly as written" / "Awaiting your review"), the exact body that will go out with `[text](url)` links rendered as clickable anchors (`mdRich`), and an **Edit draft** button (textarea + Save/Link/Discard, saves back to the scheduled message). Template dropdown hides while a send is pending.
