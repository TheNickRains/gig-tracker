...
**Needs you** (top of page) — the attention list, computed from DATA not localStorage: (1) **send awaiting review** (scheduled message hit its time with auto-send off), (2) **"{Contact} replied — your move"** (their last word is newer than yours; clears automatically when the worker logs your outbound), (3) **follow-up due** (Pitched ≥7d quiet). Rows: colored icon chip + venue + what happened + AI summary preview; tap → detail. Count pill in the label. Every notification type dismisses via a marker activity (kind:system): bounce/reply rows have **Dismiss** (a newer event re-surfaces them); follow-up rows have both **Snooze** (`✓ Follow-up snoozed` — restarts the 7-day quiet clock, nag returns after another 7 quiet days) and **Dismiss** (`✓ Follow-up dismissed` — permanent for that deal; a NEW outbound send re-arms the nags). Snooze/dismiss also suppress the "Follow up · Xd overdue" chip in Pipeline and lower priorityScore, since both go through `followupDue()`. **Bulk dismiss**: a Select button (right of the label) enters select mode — the list uncaps to ALL items (no +N overflow), rows get checkboxes (tap toggles instead of opening detail), a bar offers Select all / Clear all and "Dismiss N selected" (one batch insert of the per-type markers). Non-dismissable rows (send awaiting review) dim at 45% in select mode. Empty state: "All clear — nothing waiting on you." NO badges on nav icons anywhere (rejected: stress-inducing).
