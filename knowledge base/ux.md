...
**Org path (2026-08-24)** — step 1 has a third full-width card, **"Agency / Promoter / Buyer"** (briefcase icon, info-tip: buyers who book you into rooms they don't own). Branches the wizard: step 2 "Who are they?" (company name*, what-are-they select* [Booking agency/Promoter/Festival organizer/Management/Other], city/state, phone/website, "they mostly book" soft/hard/both) → step 3 contact (retitled "Your person there — the contact IS the relationship", no pay fields) → step 4 notes (no clientele). Submit writes a REAL `orgs` row (migration 035 — not a fake venue), links the person via `people.org_id`, and starts the deal via `pipeline_entries.org_id` (venue_id now nullable; check constraint requires one counterparty). Org deals wear the venue shape everywhere downstream (loadPipeline + `/ai/draft` map org name/type/place/books into the deal), routing them into the promoter/festival playbooks and tone cells; the AI prompt labels them BUYER, not VENUE. Orgs don't appear in Discover (v1).
