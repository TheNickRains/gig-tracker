...
**Search** — input row at top; matches venue/contact/city **plus stage names and the deal's notes + conversation text**. **Filter** — two selects beside the search: stage (All · {stage} · counts; chips were tried and REJECTED as noisy) and **location** (`#pipe-loc`, state-first hierarchy in ONE select: Anywhere, then an optgroup per state containing "All {State} · N" + its cities with counts; values `st:TN` / `ci:Memphis|TN`; options mirror the deals actually in the pipeline, persisted via `gc_pipe_loc`, self-clears on stale/legacy values). **Defaults every visit: filter All + sort Recent** (search cleared; view type + location persist). **Sort** — select: Recent (default) / Priority / Name (hidden while grouped). **Views** — list / 2-col grid toggle (persisted) + the Group-by toggle.
