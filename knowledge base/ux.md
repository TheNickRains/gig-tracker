...
Two large buttons: **"They pay you a rate"** (glass icon — bar, restaurant, winery, hotel; "soft ticket" taught in the description) · **"You sell tickets"** (ticket icon — listening room, theater, club; "hard ticket" in the description). Each jargon term carries an `.info-tip` — a little "i" (hover on desktop, tap on touch via tabindex/focus) that teaches the economics ("entry is free, the venue pays you…" / "your draw is the product…"). **Education over obfuscation is the standing principle: keep industry terms, teach them in place with `.info-tip` rather than hiding them.** Selection persists to step 2 and determines venue type options. Everywhere else hard/soft surfaces (Discover filter, venue badge, edit selects, deal flip toast) the labels are "Flat rate (soft)" / "Ticketed (hard)" — plain words first, jargon in parentheses.
